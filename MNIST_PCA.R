...
MNIST_PCA <- function(testX, z_code) {
    ...
    for(comp_value in z_list){ 
          ...
          write.csv(pca_test_z, file = paste0("PCA_test_z_", comp_value, ".csv"), row.names = FALSE)
          write.csv(pca_test_recon, file = paste0("PCA_test_output_", comp_value, ".csv"), row.names = FALSE)
